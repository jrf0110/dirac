...
dirac.sync = function(callback){
  dirac.dals.dirac.createIfNotExists(function(error){
    ...
    dirac.createTables(function(error){
      ...
      dirac.dirac.query($query, function(error, results){
        ...
        for (var table in newDb){
          ...
          for (var columnName in newDb[table]){
            ...
            if (!(columnName in currentDb[table])){
              ...
              dirac.dirac.query($alter, function(error){
    ...
    (function(table){
      fns.push(function(done){
        dirac.dirac.insert({
          tableName: table
        , version: version
        , schema: JSON.stringify(dirac.dals[table].schema)
        }, done);
      });
    })(key);
  }
...
dirac.createTables = function(callback){
  ...
  for (var i = 0; i < ordered.length; i++){
    (function(table){
      fns.push( function(done){
        dirac.dals[table].createIfNotExists(function(error, results){ done(error, results) });
      });
    })(ordered[i]);
  }
  async.parallel(fns, callback);
...
dirac.register = function(definition){
  ...
  return dirac.dals[ definition.name ] = new dirac.DAL( dirac.options.connString, definition );
};