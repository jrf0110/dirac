...
db.init = function(options){
  ...
  if ( !(connString in options) ){
    throw new Error('Dirac.init options requires property `connString`');
  }
...
db.register = function(definition){
  ...
  return db.dals[ definion.name ] = new db.DAL( db.options.connString, definition );
};
