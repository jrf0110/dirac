...
dirac.query = function( $query, callback ){
  var query = utils.mosql.sql( $query );
...
dirac.tx = {
  create: function() {
    ...
    , exec: function( callback ){
        if( !client ) throw new Error('This transaction has already transpired');

        var tasks = [ dirac.query.bind(this, { expression: 'begin' }) ];
        tasks = tasks.concat(tx._queries.map(function(query) {
          return dirac.query.bind(this, query);
        }));

        utils.async.series(tasks, function(err, results){
          dirac.query({ expression: 'commit' }, function(err) {
            client.end();
            delete client;
            callback(null);
          });
    ...
    Object.keys( dirac.Dals ).forEach( function( tbl ){
      var Dal = dirac.Dals[ tbl ].extend({
        query: function($query){
          tx._queries.push($query);
          return true;
        }
      });
