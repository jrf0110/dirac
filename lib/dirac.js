...
, alterColumnStrategies = require('./alter-column-strategies')
;
...
dirac.init = function(options){
  ...
  // Run `use` functions
  for ( var i = 0, l = dirac.afterInits.length; i < l; ++i ){
        ...
        for (var table in newDb){
          ...
          for (var columnName in newDb[table]){
            ...
            for (var attr in newColumn){
              if ( !alterColumnStrategies.has( attr ) ) continue;

              actionResult = alterColumnStrategies.get( attr ).fn(
                columnName, newColumn, oldColumn, table
              );

              if ( actionResult ) $alter.action.alterColumn.push( actionResult );
            }

            // No change made, do not query
            if ($alter.action.alterColumn.length == 0){
              continue;
            }
...
dirac.register = function(definition){
  ...
  if ( definition.type == 'view' ){
    view = dirac.views[ definition.alias || definition.name ] = utils.clone( definition );
  ...
  ] = new DAL( dirac.options.connString, definition.type == 'view' ? view : definition );
};
