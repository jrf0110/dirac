...
, raw: function raw(query, values, callback){
    ...
    // Use provided client or use random pool client
    if ( this.client ) {
      this.client.query( query, values, function( error, result ) {
        // Note: consumer must client.end() because there may
        // be subsequent queries
