...
//     this.runBeforeFilters( $query.type, function( error ){
//       if ( error ) return callback( error );
...
//           return this_.runAfterFilters( $query, result.rows, callback );
//         }
...
, query: function query($query, callback){
    ...
    console.log("Callee.name", arguments.callee.caller.name)
...
, runBeforeFilters: function( type, callback ){
    // return call
  }
});